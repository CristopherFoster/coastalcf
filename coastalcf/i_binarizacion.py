...
def plot_histogramas_con_umbral(img_db_valid, thresholds, titles, sentinel_id, output_dir, save_plots=False):
    ...
    utilizando Seaborn para una visualización más moderna.
    ...
    # Histograma general (posición 0)
    ax = axes[0]
    counts_0, bins_0, patches_0 = ax.hist(img_db_valid.ravel(), bins=300, range=[-5, 20],
                                          color='blue', alpha=0.8, edgecolor='black', linewidth=0.3)
    ...
    ax.plot(bin_centers_0, kde_scaled_0, color='black', linewidth=3.5)

    ax.set_title('Gamma0 VH band Histogram', fontsize=25)
    ax.set_xlabel('Gamma0 VH band (dB)', fontsize=13 )
    ax.set_ylabel('Frequency', fontsize=13)
    ax.set_ylim(0, 10000*8.5) #*2.5 6000
    ax.legend(["Smoothed Distribution"], loc='upper right', fontsize=14)

    # Histogramas con umbrales
    for idx, (thresh, title) in enumerate(zip(thresholds, titles)):
        ax = axes[idx + 1]
        counts, bins, patches = ax.hist(img_db_valid.ravel(), bins=300, range=[-5, 20],
        ...
        ax.plot(bin_centers, kde_scaled, color='black', linewidth=3.5)

        # Colorear bins según el umbral
        for bin_left, patch in zip(bins[:-1], patches):
            if bin_left < thresh:
                patch.set_facecolor('purple')  # naranja oscuro
            else:
                patch.set_facecolor('pink')

        # Línea de umbral
        ax.axvline(thresh, color='red', linestyle='--', label=f'Threshold:\n{thresh:.2f} dB')
        ...
        ax.text(0.05, 0.95, f"< {thresh:.2f} dB:\n{perc_above:.1f}%",
                transform=ax.transAxes, fontsize=16,
                verticalalignment='top', color='purple')
        ax.text(0.75, 0.95, f"> {thresh:.2f} dB:\n{perc_below:.1f}%",
                transform=ax.transAxes, fontsize=16,
                verticalalignment='top', color='deeppink')

        ax.set_title(f"{title} Threshold", fontsize=25)
        ax.set_xlabel("Gamma0 VH band (dB)", fontsize=13)
        ax.set_ylabel("Frequency", fontsize=13)
        ax.set_ylim(0, 10000*8.5)#*2.5
...
def li_iterative_threshold2(image, extra_initial_value=None):
    li_val = threshold_li(image)
    li_iter_val = li_val + 3
...
def plot_threshold_histograms_extended(img_db_valid, thresholds, titles, output_dir, sentinel_1_path, extent=None):
    import matplotlib.pyplot as plt
    import numpy as np
    from scipy.stats import gaussian_kde
    from matplotlib import cm
    from skimage.filters import threshold_sauvola, threshold_niblack
    from .a_sentinel1_id import get_sentinel_fecha_id
    ...
    # 1. Iterative Li/MCET Threshold Optimization
    li_val, li_iter_val, li_iter_history = li_iterative_threshold2(img_db_valid, extra_initial_value=4.1)
    ...
    axes[0].plot(li_iter_history, entropies_iter, marker='o', color='maroon', label='MCET Iterative')
    axes[0].axvline(li_val, color='red', linestyle='--', label='MCET Threshold')
    axes[0].set_title("MCET Iterative (Cross Entropy)",fontsize=25)
    axes[0].set_xlabel("Gamma0 VH band (dB)",fontsize=13)
    axes[0].set_ylabel("Cross Entropy",fontsize=13)
    ...
    axes[1].set_title("Intra-class Variance vs Threshold (Otsu)",fontsize=25)
    axes[1].set_xlabel("Gamma0 VH band (dB)",fontsize=13)
    axes[1].set_ylabel("Intra-class Variance",fontsize=13)
    axes[1].legend(fontsize=18)
    axes[1].grid(True)

    # 3. Sauvola Threshold Map
    sauvola_thresh = threshold_sauvola(img_db_valid, window_size=15, k=0.2)
    im2 = axes[2].imshow(sauvola_thresh, cmap='gnuplot', extent=extent)

    axes[2].ticklabel_format(axis='x', style='plain', useOffset=False)  # sin notación científica
    axes[2].tick_params(axis='x', labelrotation=15)                     # ladea 15°
    axes[2].set_title("Local Threshold Map – Sauvola",fontsize=25)
    ...
    im3 = axes[3].imshow(niblack_thresh, cmap='gnuplot', extent=extent)

    axes[3].ticklabel_format(axis='x', style='plain', useOffset=False)
    axes[3].tick_params(axis='x', labelrotation=15)
    axes[3].set_title("Local Threshold Map – Niblack",fontsize=25)
    ...
    axes[4].set_ylim(0, 10000*8.5)
    axes[4].set_xlabel("Local Threshold Value (dB)",fontsize=13)
    axes[4].set_ylabel("Frequency")
    axes[4].set_title("Histogram of Local Thresholds",fontsize=25)
    axes[4].legend(fontsize=18)
    axes[4].grid(True)
    ...
    axes[5].set_title("Global VH Histogram with Selected Thresholds",fontsize=25)
    axes[5].set_xlabel("Gamma0 VH band (dB)",fontsize=13)
    axes[5].set_ylabel("Frequency",fontsize=13)
    axes[5].set_ylim(0, 10000*8.5)
    axes[5].legend(fontsize=18)
    axes[5].grid(True)

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{sentinel_id}_ThresholdsVisualization.png"), dpi=300)
    return fig
...
def i_binarizacion(
    ...
    w = producto_db.getSceneRasterWidth()
    h = producto_db.getSceneRasterHeight()

    # ---------- leer datos lineales ----------------------
    img_db_data = np.zeros(w * h, np.float32)
    banda_db.readPixels(0, 0, w, h, img_db_data)
    # Geocodificación de la escena
    gc = producto_db.getSceneGeoCoding()   # <-- cambio clave

    # Esquinas del raster
    geo_ul = gc.getGeoPos(PixelPos(0, 0), None)                       # esquina superior-izquierda
    geo_lr = gc.getGeoPos(PixelPos(w-1, h-1), None)          # esquina inferior-derecha

    # Extent para matplotlib (xmin, xmax, ymin, ymax)
    extent = [geo_ul.getLon(), geo_lr.getLon(), geo_lr.getLat(), geo_ul.getLat()]
    ...
    def li_iterative_threshold(
        ...
        ax.plot(li_iter_history, entropies_iter, marker='o', linestyle='-', color='C1', label='MCET Iterative path')
        ...
        ax.set_title('Optimization path: MCET Iterative (Cross Entropy)', fontsize=25)
        ax.set_xlabel('Threshold (dB)', fontsize=13)
        ax.set_ylabel('Cross Entropy', fontsize=13)
    ...
    bd_MCET = crear_band_descriptor('flood_MCET', expr_li)
    #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    bd_MCET_iter = crear_band_descriptor('flood_MCET_iter', expr_li_iter)
    ...
    targetBands[3] = bd_MCET
    targetBands[4] = bd_MCET_iter
    ...
    #li_val, li_iter_val, li_iter_history = li_iterative_threshold(img_db_valid, extra_initial_value=4.1)
    ...
    thresholds = [otsu_val, niblack_val, sauvola_val, li_val, li_iter_val]
    titles = ['Otsu', 'Niblack', 'Sauvola', 'MCET', 'MCET Iterative']
    fig = plot_threshold_histograms_extended(img_db_valid, thresholds, titles, output_dir, sentinel_1_path, extent)
    #plt.suptitle("Visualizaciones Comparativas de Umbrales", fontsize=18, y=1.02)
    plt.suptitle("Comparative Threshold Visualization", fontsize=20, y=1.02)
    plt.show()
    ...
    return productos_binarios, {
        ...
        "sauvola": sauvola_val,
        "MCET": li_val,
        "MCET_iterativo": li_iter_val
    }, img_db_valid_product, extent  
